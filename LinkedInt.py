...
csv.append('"Firstname","Lastname","Email","Occupation","Location","Start Date","Company ID"')
...
def generateEmail(data_firstname,data_lastname):
    ...
    titleList = ['ing','eng','cissp','crha','chrp','cpa','dr','mba','crisc','pcp','phd']
...
def linkedinGetSearchPeople(companyID, page):
    ...
    for c in content['elements'][0]['elements']:
        ...
        if 'com.linkedin.voyager.search.SearchProfile' in c['hitInfo'] and c['hitInfo']['com.linkedin.voyager.search.SearchProfile']['headless'] == False:
            ...
            "<td>{}</td>" \
            "<td>{}</td>" \
            "<a>".format(data_slug, data_picture, data_slug, email, data_occupation, data_location,seniority,companyID)

            # Add a csv entry for that outpup
            csv.append('"{}","{}","{}","{}","{}","{}","{}"'.format(data_firstname, data_lastname, email, data_occupation, data_location,seniority,companyID.replace(",",";")))
